...
def preprocess(imputed_data, processed_savepath=None, isTrain=False, model_path=None, model_features=None):
    ...
    if isTrain:
        ...
        if model_path:
            model_features = pickle.load(open(model_path, 'rb')).get_booster().feature_names
        elif model_features:
...
def make_predictions(preprocessed_data, prediction_savepath, model_path):
    ...
    preprocessed_data['probabilities'] = model.predict_proba(preprocessed_data[model_features])[:, 1]
    save_columns = ['chr_pos_ref_alt', 'Consequence', 'PHRED', 'probabilities']
    save_file = preprocessed_data[save_columns]
    save_file = save_file.sort_values("probabilities").drop_duplicates('chr_pos_ref_alt')
...
def check_input_columns(input_path):
    ...
    input['chr_pos_ref_alt'] = ['_'.join([str(ele) for ele in item]) for item in input[['#Chrom', 'Pos', 'Ref', 'Alt']].values]
    return input
...
if __name__ == "__main__":
    ...
    make_predictions(preprocessed_data, args.prediction_savepath, args.model_path)
    # sys.stdout.close()
