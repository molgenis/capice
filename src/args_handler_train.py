...
class ArgsHandlerTrain:
    ...
    def _handle_args(self, args):
        ...
        if args.output:
            ...
            output_path = re.sub(r"(\.tsv|\.tsv\.gz)$", "_capice.dat", input_path)
    ...
    def _validate_args(self, args):
        ...
        if not (input_path.endswith(".tsv") or input_path.endswith(".tsv.gz")):
            self.parser.error(f"input '{input_path}' is not a .tsv or .tsv.gz file")
        ...
        if not impute_path.endswith(".json"):
            self.parser.error(f"impute file '{impute_path}' is not a .json file")
        ...
        if test_split < 0 or test_split > 1:
            self.parser.error(f"test split '{test_split}' is not between 0 and 1")

        if args.output:
            output_path = args.output[0]
            if not (output_path.endswith(".tsv") or output_path.endswith(".dat")):
