...
class TestInputChecker(unittest.TestCase):
    ...
    def tearDownClass(cls):
        print('Tearing down.')
        to_remove = ['.another_test_log_output', '.test_log_output', '.another_test_output']
    ...
    def test_input_error(self):
        ...
        self.assertTrue('Input file does not exist!'.startswith(str(context.exception)))

    def test_create_output_loc(self):
        print('Creating output location')
        dir_to_be_created = os.path.join(get_project_root_dir(), '.another_test_output')
        self.input_checker.check_output_loc(dir_to_be_created)
        self.assertTrue('.another_test_output' in os.listdir(get_project_root_dir()))
    ...
    def test_log_checker_both(self):
        ...
        logchecker = LogChecker(log_loc=log_output, output_loc=self.output_directory)
        logchecker.check_log_loc()
        self.assertTrue('.test_log_output' in os.listdir(get_project_root_dir()))
    ...
    def test_log_checker_output(self):
        ...
        logchecker.check_log_loc()
        self.assertTrue('.another_test_log_output' in os.listdir(get_project_root_dir()))
